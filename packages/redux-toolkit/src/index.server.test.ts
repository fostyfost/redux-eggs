...
describe('Tests for `createStore` with Redux Toolkit (Server-side)', () => {
  ...
  test('Store is initialized with middlewares from extensions', () => {
    ...
    const store = createStore({ extensions: [extension1, extension2] })

    const mid = expect.any(Function)
    // noinspection JSDeprecatedSymbols
    const defaultMiddlewares = ReduxToolkit.getDefaultMiddleware().map(() => expect.any(Function))
    expect(spyOnApplyMiddleware).toBeCalledTimes(1)
    expect(spyOnApplyMiddleware).toBeCalledWith(mid1, mid3, mid2, mid4, ...defaultMiddlewares, mid)
  ...
  test('Options for default middlewares should work', () => {
    const spyOnApplyMiddleware = jest.spyOn(Redux, 'applyMiddleware')
    ...
    expect(spyOnApplyMiddleware).toBeCalledTimes(1)
    expect(spyOnApplyMiddleware).toBeCalledWith(mid)
    spyOnApplyMiddleware.mockClear()
    ...
    const thunkMiddleware = expect.any(Function)

    expect(spyOnApplyMiddleware).toBeCalledTimes(1)
    expect(spyOnApplyMiddleware).toBeCalledWith(thunkMiddleware, mid)
    spyOnApplyMiddleware.mockClear()

    spyOnApplyMiddleware.mockRestore()
  })

  test('Store is initialized with custom `enhancers` from extensions', () => {
    const spyOnCompose = jest.spyOn(Redux, 'compose')
    ...
    createStore({ extensions: [{ enhancers: [enhancer1] }, { enhancers: [enhancer2] }] })

    expect(spyOnCompose).toBeCalledTimes(1)
    expect(spyOnCompose).toBeCalledWith(enhancer1, enhancer2, applyMiddlewareResult)
  ...
  test('Store is initialized with compose as `composeMiddlewares`', () => {
    const spyOnCompose = jest.spyOn(Redux, 'compose')

    expect(spyOnCompose).not.toBeCalled()
    ...
    expect(spyOnCompose).toBeCalledTimes(1)
    expect(spyOnCompose).toBeCalledWith(result1, result3, result2)
    spyOnCompose.mockClear()

    store.removeEggs([egg1])
    store.dispatch({ type: 'test' })
    expect(spyOnCompose).toBeCalledTimes(1)
    expect(spyOnCompose).toBeCalledWith(result2)
    spyOnCompose.mockClear()

    store.removeEggs([egg2])
    store.dispatch({ type: 'test' })
    expect(spyOnCompose).toBeCalledTimes(1)
    expect(spyOnCompose).not.toBeCalledWith(expect.anything())
    expect(Object.keys(store.getState())).toEqual([])
  ...
  test('Extensions can enhance store', () => {
    ...
    const store = createStore<Store & Ext>({
      extensions: [{ enhancers: [enhanceStore1] }, { enhancers: [enhanceStore2] }],
    })
